		...
		public async Task getSelectionPhase(Move currentMove) {
			...
			if (Physics.Raycast(ray, out hit, 1000.0f)) {
				...
				if (selectedItem is Tile) {
					...
					if (selectedUnit.getCharacter(battlefield) == this.character && (!selectedUnit.hasMovedThisTurn || (!selectedUnit.hasAttackedThisTurn && selectedUnit.getTargets(tileCoords.x, tileCoords.y, battlefield, character).Count != 0))) {
